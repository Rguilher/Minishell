...
static void	cd_parameter(char **token, char *old_pwd)
{
	...
	if (!ft_strncmp(token[1], "-", 2))
	{
		...
		if (!new_pwd)
		{
			print_error("sh: ", "cd", "OLDPWD not set", 1);
			return ;
		}
		free(token[1]);
	...
	if (chdir(token[1]) == -1)
	{
		if (!token[1][0])
			return ;
		print_error("sh: cd: ", token[1], strerror(errno), 1);
	}
	else
...
static void	cd_home(char *old_pwd)
{
	...
	if (!new_pwd)
	{
		print_error("sh: ", "cd", "HOME not set", 1);
		return ;
	}
	if (chdir(new_pwd) == -1)
		print_error("sh: cd: ", new_pwd, strerror(errno), 1);
	else
	{
		update_pwd_or_oldpwd(new_pwd, "PWD=", 4);
		update_pwd_or_oldpwd(old_pwd, "OLDPWD=", 7);
...
void	cd(char **token)
{
	...
	if (!token[1] || !ft_strncmp(token[1], "--", 3))
		...
		print_error("sh: ", "cd", "too many arguments", 1);
	free(old_pwd);
}
